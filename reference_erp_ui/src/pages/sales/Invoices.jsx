...
export default function Invoices() {
    ...
    try {
      ...
      setInvoices(
        res.docs
          ...
          .filter((row) => row.invoiceKind !== 'purchase' && row.customerName)
      );
    } catch (error) {
      console.error(error);
      toast.error('Satış faturaları yüklenemedi');
    } finally {
      setLoading(false);
    }
  };
  ...
  const updatePaymentStatus = async (id, status) => {
    try {
      ...
      toast.success(`Ödeme durumu ${status} olarak güncellendi`);
      load();
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="anim-fade" style={{ padding: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32 }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 900, color: '#fff', margin: 0 }}>Satış Faturaları</h1>
          <p style={{ color: '#475569', fontSize: 14, marginTop: 4 }}>Sevkiyat bağlantılı kesilen faturalar ve tahsilat takibi</p>
        </div>
      </div>
      ...
      <div style={{ background: '#0d1117', border: '1px solid #1e293b', borderRadius: 12, padding: 16, marginBottom: 24, display: 'flex', gap: 12 }}>
        <div style={{ position: 'relative', flex: 1 }}>
          ...
          <input
            style={{ width: '100%', height: 38, padding: '0 12px 0 36px', background: '#0a0f1e', border: '1px solid #334155', borderRadius: 6, color: '#e2e8f0', fontSize: 13, outline: 'none' }}
      ...
      <div style={{ background: '#0d1117', border: '1px solid #1e293b', borderRadius: 12, overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={7} style={{ padding: 100 }}>
                  <EmptyState message="Satış faturası bulunamadı." />
                </td>
              </tr>
            ) : (
              filtered.map((invoice) => (
                <tr key={invoice.id}>
                  ...
                  <td style={TD}>{invoice.partNumber} x {invoice.quantity}</td>
                  <td style={TD}>{formatDate(invoice.invoiceDate || invoice.createdAt)}</td>
                  <td style={{ ...TD, textAlign: 'right', fontWeight: 900, color: '#f1f5f9' }}>
                    {formatNumber(invoice.totalAmount || invoice.amount)} {invoice.currency || 'USD'}
                  </td>
                  <td style={TD}>
                    <select
                      disabled={!canEdit}
                      style={{ height: 28, background: '#0a0f1e', border: '1px solid #334155', color: invoice.paymentStatus === 'Paid' ? '#34d399' : '#fbbf24', fontSize: 11, fontWeight: 800, borderRadius: 4, padding: '0 8px' }}
